...
class TurtlebotWorld():
    def __init__(self, beam_num, index, num_env):
        ...
        self.goal_sub = rospy.Subscriber('move_base_simple/goal', PoseStamped, self.cb_get_goal)
        ...
        # self.trainSimName = rospy.get_param('/train_simulator','realWorld')
        # self.gazeboFlag = False
        # if(self.trainSimName == 'gazebo'):
        #     self.gazeboFlag = True
        self.velscale = 1.0
        self.realWorld = rospy.get_param('/real_world',True)
    ...
    def get_laser_observation(self):
        ...
        # transformed_scan = np.concatenate((scan[180:360],scan[0:180]), axis=0)

        # scan = copy.deepcopy(transformed_scan)
        ...
        for x in xrange(int(sparse_beam_num / 2)):
            ...
            index += step
        sparse_scan_right = []
        index = sparse_beam_num / 2
        for x in xrange(int(sparse_beam_num / 2)):
            sparse_scan_right.append(scan[int(index)])
            index += step
        scan_sparse = np.concatenate((sparse_scan_left, sparse_scan_right), axis=0)
        normalized_scan= scan_sparse / 3.5 - 0.5
        #print("normalized_scan",normalized_scan)
    ...
    def control_vel(self, action):
        ...
        move_cmd.angular.z = self.velscale*action[1]
